...
href hashmapGet(uchar* heap, href mapIndex, href key) {
    ...
    uint searchLimit = MAP_MAX_SEARCH < capacity ? MAP_MAX_SEARCH : capacity;
    for(uint i = hashIndex, j = 0; j < searchLimit; i = (i + 1) % capacity, j++) { //i = search location (array index) | j = search count
...
bool resizeHashmap(uchar* heap, uint maxHeapSize, href mapIndex, uint newCapacity) {
    ...
    for(int i = 0; i < oldCapacity; i++) {
        ...
        for(uint slot = keyHash % newCapacity, j = 0; j < searchLimit; slot = (slot + 1) % newCapacity, j++ ) {
